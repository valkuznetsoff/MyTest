...
// Проверка подключа на соответствие условиям поиска
            if SearchStr > '' then begin
              if Pos(SearchStr, LowerCase(subKeys[i])) > 0 then
              begin
                FoundKeys.Add(s);
                Inc(iFoundKeys);
              end;
            ...
            Write(#13, 'Проверено: '+IntToStr(iCheckKeys)+' ключей. Найдено соответствий: '+IntToStr(iFoundKeys));
...
// Если нажата клавиша Esc
            if KeyPressed = 27 then begin
              ...
              FoundKeys.Add('Проверено: '+IntToStr(iCheckKeys)+' ключей. Найдено соответствий: '+IntToStr(iFoundKeys));
              FoundKeys.Add('Остановка пользователем.');
            end;

// Поиск внутри подключа
            if (not Cancel) and (subKeys[i] > '') and (reg.HasSubKeys)
              then FindKey(s);
            inc(i);
          end;
        finally
...
// Параметры командной строки
  ...
  FoundKeys.Add('Раздел реестра: '+StartKey);
  FoundKeys.Add('Искомая строка: '+SearchStr);
// Проверка начального ключа
  if SearchStr > '' then begin
    if Pos(SearchStr, LowerCase(StartKey)) > 0 then
    begin
